...
class Arch:
    ...
    @classmethod
    def mutate_arch(cls, arch, edits):
    ...
    @classmethod
    def get_path_indices(cls, arch, long_paths=True):

        paths, contains_long_path = cls.get_paths(arch)
    ...
    @classmethod
    def encode_paths(cls, arch, long_paths=True):
        # given a path in the darts encoding, output its path encoding
        path_indices, _ = cls.get_path_indices(arch=arch, long_paths=long_paths)
        num_ops = len(OPS)
